...
export type DurationVariance = 'over' | 'match' | 'short' | 'unknown';
...
/** 正式な予定工数。0・負数・不正値は未入力として null を返す。 */
export function getPlannedMinutes(task: Task): number | null {
  const minutes = task.quantity * task.time_per_unit;
  return Number.isFinite(minutes) && minutes > 0 ? minutes : null;
}

/** 予定区間と指定期間の重なり比率で、予定工数を按分する。 */
export function getPlannedMinutesForRange(task: Task, rangeStart: Date, rangeEnd: Date): number {
  const plannedMinutes = getPlannedMinutes(task);
  const scheduledStart = validDate(task.scheduled_start);
  const scheduledEnd = validDate(task.scheduled_end);
  if (plannedMinutes === null || !scheduledStart || !scheduledEnd || scheduledEnd <= scheduledStart) return 0;

  const totalRangeMinutes = (scheduledEnd.getTime() - scheduledStart.getTime()) / 60000;
  const overlappedMinutes = overlapMinutes(scheduledStart, scheduledEnd, rangeStart, rangeEnd);
  return plannedMinutes * (overlappedMinutes / totalRangeMinutes);
}
...
function getTaskSessions(task: Task, sessions: TaskSession[]): TaskSession[] {
  return sessions.filter(session => session.task_id === task.id);
}
...
/**
 * 実績工数。セッションを最優先し、次に actual_time、最後に actual_start〜actual_end を使う。
 * 予定時間は代用しない。実績追跡対象外・実績情報なしは null、未着手は 0 を返す。
 */
export function getActualMinutes(task: Task, sessions: TaskSession[], now = new Date()): number | null {
  if (!task.track_actual) return null;
  if (task.status === 'not_started') return 0;

  const taskSessions = getTaskSessions(task, sessions);
  if (taskSessions.length > 0) {
    let totalMinutes = 0;
    for (const session of taskSessions) {
      const start = validDate(session.session_start);
      const end = sessionEnd(task, session, now);
  ...
  if (Number.isFinite(task.actual_time) && task.actual_time > 0) return task.actual_time;

  const actualStart = validDate(task.actual_start);
  const actualEnd = validDate(task.actual_end);
  if (actualStart && actualEnd && actualEnd > actualStart) {
    return (actualEnd.getTime() - actualStart.getTime()) / 60000;
  }
  return null;
}

/** 実績工数のうち、指定期間に属する分。内部計算では丸めない。 */
export function getActualMinutesForRange(
  task: Task,
  ...
  if (taskSessions.length > 0) {
    ...
    for (const session of taskSessions) {
      ...
      const end = sessionEnd(task, session, now);
      if (start && end && end > start) {
        totalMinutes += overlapMinutes(start, end, rangeStart, rangeEnd);
      }
    }
    return totalMinutes;
  }
  ...
  const actualStart = validDate(task.actual_start);
  const actualEnd = validDate(task.actual_end);
  if (Number.isFinite(task.actual_time) && task.actual_time > 0) {
    if (actualStart && actualEnd && actualEnd > actualStart) {
      const grossMinutes = (actualEnd.getTime() - actualStart.getTime()) / 60000;
      const overlappedMinutes = overlapMinutes(actualStart, actualEnd, rangeStart, rangeEnd);
      return task.actual_time * (overlappedMinutes / grossMinutes);
    }

    const attributionDate = actualStart ?? validDate(task.completed_at);
    return attributionDate && attributionDate >= rangeStart && attributionDate < rangeEnd
      ? task.actual_time
      : 0;
  }

  if (actualStart && actualEnd && actualEnd > actualStart) {
    return overlapMinutes(actualStart, actualEnd, rangeStart, rangeEnd);
  }
  return null;
}

/** 指定日の実績工数。内部計算では丸めない。 */
export function getActualMinutesForDay(
  task: Task,
  sessions: TaskSession[],
  dayDate: Date,
  now = new Date(),
...
export function getDurationVariance(
  ...
  const plannedMinutes = getPlannedMinutes(task);
  const actualMinutes = getActualMinutes(task, sessions);
  if (plannedMinutes === null || actualMinutes === null) return 'unknown';
  const difference = actualMinutes - plannedMinutes;
  if (difference > toleranceMinutes) return 'over';
  if (difference < -toleranceMinutes) return 'short';
  return 'match';
}
  ...
  if (taskSessions.length > 0 && (task.status === 'completed' || task.status === 'suspended' || task.status === 'in_progress')) {
    ...
    for (const s of sorted) {
      ...
      if (s.session_end) {
        ...
        // 現在進行中タスクの open セッション: 現在時刻まで計算
        sEnd = new Date();
      } else {
        // completed/suspended タスクの open セッションは孤立データ: スキップ
        continue;
      }
      // 当日にクリップ
