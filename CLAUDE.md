...
- タスクの所要時間超過・短縮要因を分析して自己改善したいビジネスパーソン
...
| 見積時間との差異を振り返れない | 所要時間超過・短縮の要因を記録し、統計集計 |
...
| 日時処理 | `src/lib/dateTime.ts`（`Asia/Tokyo` 固定）+ ネイティブ `Date` / `Intl.DateTimeFormat` |
...
| Edge Functions | `supabase/functions/update-account`（ログイン中ユーザーのメールアドレス/パスワード更新） |

### 認証方式

Supabase **メールアドレス + パスワード認証**。メール確認は OFF。ソーシャルログイン・マジックリンクは未実装。設定画面からメールアドレス/パスワードを変更する場合は `update-account` Edge Function を経由する。
...
| `SUPABASE_SERVICE_ROLE_KEY` | `update-account` Edge Function 用のサービスロールキー。フロントエンド `.env` ではなく Supabase Functions Secrets に設定する |

> **注意**: `VITE_` 変数はフロントエンド用。`SUPABASE_SERVICE_ROLE_KEY` はブラウザに公開してはならず、Supabase Functions Secrets にのみ設定する。AI エージェントは既存の環境変数を変更・削除・上書きしてはならない。
...
│   │   ├── dateTime.ts           # Asia/Tokyo 固定の入力変換・表示・日/月境界
│   │   ├── utils.ts              # 汎用ユーティリティ（工数計算・ツリー構築・CSV出力など）
...
│       └── SettingsPage.tsx      # 設定（アカウント情報・カテゴリ管理・通知許可）
│
├── supabase/
│   ├── migrations/               # SQL マイグレーションファイル（番号順に適用）
│   └── functions/
│       └── update-account/        # Auth ユーザーのメール/パスワード更新 Edge Function
│
├── public/                       # 静的ファイル
├── .env                          # 環境変数（Git 管理外）
├── tailwind.config.js
...
| `AuthContext` | `src/contexts/AuthContext.tsx` | Supabase 認証状態。メール/パスワードのアカウント情報更新は `update-account` Edge Function を呼び出す。 | `user`, `session`, `loading`, `signIn`, `signUp`, `signOut`, `updateAccount` |
...
Supabase クライアント（`src/lib/supabase.ts`）を Context 内で直接呼び出す。データ取得は Context の初期化時（`useEffect`）で行い、以後は Context 内の state を更新する。例外として、アカウント情報更新は `AuthContext.updateAccount()` が `supabase/functions/update-account` を `fetch` で呼び出す。
...
| `start_delay_factor` | `string \| null` | 旧開始遅延要因。履歴保持のためDBに残すが、現在のUIでは参照・更新しない |
| `start_early_factor` | `string \| null` | 旧早期開始要因。履歴保持のためDBに残すが、現在のUIでは参照・更新しない |
...
| `ends_next_day` | `boolean` | 終了時刻を開始日の翌日として扱うか |
...
| `20260621093000_add_recurrence_ends_next_day.sql` | 定常タスクグループへ `ends_next_day` カラム追加 |
...
| ダイアログ | 表示タイミング | 記録内容 |
|---|---|---|
| `StartDialog` | 未着手 → 進行中 | 実績開始時刻 |
| `EndDialog` | 進行中 → 完了 | 実績終了時刻・時間超過/短縮要因 |
| `FullActualDialog` | 未着手から直接完了 | 開始・終了時刻・時間超過/短縮要因 |
...
- **未スケジュールパネル（`UnscheduledPanel`）**: 日・週・月ビュー共通でカレンダー下部に表示。`scheduled_start` または `scheduled_end` が未入力の未完了タスクを一覧表示。件数バッジ・折りたたみ機能付き。編集アイコンで `TaskForm` を開いて予定日時を設定できる。完了済みタスクは除外
...
- ステータスが `completed` の場合 → 終了実績日時（`actual_end`）が必須
- ステータスが `suspended` の場合 → 中断時間エントリが1件以上（`suspendVal` 入力済み）必須
- バリデーション失敗時はエラーメッセージを該当フィールド直下にインライン表示し、保存をブロック
- ステータスを変更するとバリデーションエラーがリセットされる

**自動計算:**
- 予定終了時刻の自動計算: `scheduled_start + quantity × time_per_unit`
- 作業量・単位時間が空の場合、予定開始/終了時刻から自動補完
- ステータスを「完了」に変更したとき、実績日時が未入力なら予定日時を自動コピー
- 「完了」ステータスで予定開始/終了を入力した際も、実績日時が未入力なら自動コピー
...
- 期間（開始日・終了日）+ プレビュー件数
- 終了時刻の「翌日終了」指定。同日終了では終了時刻が開始時刻以下の場合に保存不可
- 保存時: `TaskContext.tsx` 内の `generateDateKeys()` でJST暦日の全対象日を算出し、各日に `scheduled_start/end` 付きタスクを一括生成

### 日時ユーティリティ（`src/lib/dateTime.ts`）

- 業務タイムゾーンは `Asia/Tokyo` 固定。入力した `datetime-local` はJSTとしてUTC ISOへ変換し、表示時は明示的にJSTへ戻す
- 日境界は `[当日00:00 JST, 翌日00:00 JST)`、月境界は `[月初00:00 JST, 翌月月初00:00 JST)`
- `getJstDayRange` / `getJstMonthRange` を日別・月別集計、一覧フィルター、レポートで共用する
- `toJstWallClockDate` はカレンダー描画専用。DB保存や絶対時間差の計算には使用しない
...
| `clearAllTaskNotifications` | `() => void` | ログアウト・全件再取得時に全タイマーを取り消す |
...
| 定数配列 | UPPER_SNAKE_CASE | `DURATION_OVER_FACTORS` |
...
- `md`（768px）未満はモバイル表示とし、左サイドバーを非表示にして safe area 対応の下部固定ナビゲーションを表示する
- Galaxy S20 5G の CSS viewport 360 × 800（縦向き）を基準端末とする
- モバイルの主要ボタン・入力欄は44px以上、入力文字サイズは16pxを基準とする
- タスク・定常タスクの作成/編集はモバイルで `100dvh` の全画面フォーム、補助ダイアログは画面下寄せかつ内部スクロール可能にする
- 本文には下部ナビゲーションと safe area の分だけ余白を確保する
- カレンダーページの週/月グリッドは情報量を維持し、横スクロール対応とする
- デスクトップでは従来の左サイドバーと中央配置ダイアログを維持する
...
- `SUPABASE_SERVICE_ROLE_KEY` はフロントエンドに置かず、Supabase Functions Secrets に設定する
...
| 通知の詳細設定 | 予定開始・終了時刻の通知のみ。事前リマインダー設定は設けない |
...
| 2026-06-08 | `TaskForm` にバリデーションを追加。ステータスが「進行中」「中断」「完了」のとき開始実績日時を必須化、「完了」のとき終了実績日時を必須化、「中断」のとき中断時間（`suspendVal`）を1件以上必須化。バリデーションエラーは該当フィールド下にインライン表示。ステータス変更で全エラーをリセット |
| 2026-06-08 | `utils.ts` の `getWorkloadMinsForDay` 冒頭に `scheduled_start` / `scheduled_end` 両方必須のガードを追加。片方でも未入力のタスクは工数計算から除外（タスク一覧・カレンダーの使用時間バー・分析の日別/月別グラフに適用）。`AnalyticsPage` の `dailyData`・`monthlyData` にも同ガードを適用し計画時間の計算も除外 |
| 2026-06-08 | `CalendarPage` に `UnscheduledPanel` コンポーネントを追加。予定開始日時または予定終了日時が未入力の未完了タスクを日・週・月ビュー共通でカレンダー下部に一覧表示。件数バッジ・折りたたみ機能付き。鉛筆アイコンで編集フォームを開き予定日時を設定できる |
| 2026-06-18 | `SettingsPage` にアカウント情報変更、カテゴリ色フィルタ/検索/一括削除を追加。`AuthContext.updateAccount()` と `supabase/functions/update-account` によりログイン中ユーザーのメールアドレス/パスワード更新に対応 |
| 2026-06-20 | タスク作成・編集とステータス変更時の開始遅延/前倒し要因入力、テキスト出力項目、開始タイミング分析を削除。DBの既存要因カラムは履歴保持のため維持 |
| 2026-06-20 | `npm audit` の全脆弱性を解消。Vite 8、React plugin 6、ESLint 9、TypeScript 5.9、PostCSS 8.5等へ更新し、Node.js要件を20.19以上または22.12以上へ変更 |
| 2026-06-20 | Galaxy S20 5G（360×800）を基準にスマートフォンUIを最適化。下部固定ナビゲーション、全画面フォーム、44px操作領域、safe area/dvh対応、各画面のレスポンシブ配置、週/月カレンダーの横スクロールを追加 |
| 2026-06-21 | ブラウザ通知を仕様化。未着手の開始通知、未完了の終了通知、状態別本文、予定変更時の取消・再予約、完了/削除時の取消、重複防止、過去時刻除外を実装 |
| 2026-06-21 | 業務日時を `Asia/Tokyo` 固定へ統一。JST入力/表示、UTC保存、半開区間の日/月境界、日跨ぎ・月跨ぎ按分、終了00:00の表示、JST日付フィルター、定常タスクの翌日終了を実装 |
