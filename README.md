...
| 予定開始日時 | — | 予定の開始日時。「終日」チェックで 00:00〜23:59 に自動設定 |
| 予定終了日時 | — | 数量・所要時間が入力済みの場合は開始日時から自動計算 |
| 実績入力あり | — | OFF にすると実績フィールドが非表示になり分析対象外になる |
| 備考 | — | メモや補足情報 |
...
> **「実績のみ登録」モード**: フォーム上部の「実績のみ登録」トグルを ON にすると、予定日時を省略してすぐに実績時間だけ記録できる。
...
> **集計対象**: 「実績入力あり」が ON で、かつ親タスクを持たない（ルート）タスクのみが集計される。
...
- タスクフォームの「実績入力あり」トグルを OFF にすると、実績フィールドが非表示になり分析ページの集計対象からも除外される。会議や固定イベントなど分析に含めたくないタスクに使用する。
- 定常タスクグループの「実績入力」設定を変更すると、グループ内の**未完了タスク全件**に設定が一括反映される。

### 分析ページの集計対象について

- 分析ページが集計するのは「**実績入力あり が ON**」かつ「**親タスクを持たないルートタスク**」のみ。子タスクは集計から除外される（親タスクで二重計上しないため）。
